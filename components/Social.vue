<template>
  <Container class="items-center justify-evenly space-x-8">
    <a href="https://www.linkedin.com/in/augustin-briolon/" target="_blank">
      <img src="/icons/linkedin.svg" alt="Logo Linkedin" class="dark-fill w-8 h-8">
    </a>
    <a href="https://github.com/AugustinBriolon" target="_blank">
      <img src="/icons/github.svg" alt="Logo Github" class="dark-fill w-8 h-8">
    </a>
    <a href="https://twitter.com/AugustinBriolon" target="_blank">
      <img src="/icons/twitter.svg" alt="Logo Twitter" class="dark-fill w-8 h-8">
    </a>
  </Container>
</template>

<script>

export default {
  name: "Social",
};
</script>